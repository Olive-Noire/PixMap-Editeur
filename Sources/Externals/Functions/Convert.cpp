...
std::vector<Pixel> Convert_StringToPixelsArray(std::string pxs) {
    ...
    for (int i = 0;i*3 < s.size();i++) pxsLines.push_back(s[i*3]+" "+s[1+i*3]+" "+s[2+i*3]+" ");
    for (int i = 0;i < pxsLines.size();i++) pixels.push_back(Convert_StringToPixel(pxsLines[i]));
...
bool IsNumber(std::string n) {
    ...
    for (int i = 0;i < n.size();i++) {
        ...
        } else if (i == (n.size() - 1)) return true;
