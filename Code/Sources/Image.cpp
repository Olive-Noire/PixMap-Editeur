...
void Image::Render() {
    ...
    for (unsigned int i = 0;i < pixels.size();i++) {
        ...
        for (unsigned int j = 0;j < pixels[i].size();j++) {
            ...
            pixels[i][j].rect = {(int)(i*(width/pixels.size())),(int)(j*(height/pixels[0].size())),(int)(width/pixels.size()),(int)(height/pixels[0].size())};
...
void Image::Update() {
    ...
    if (Editor::keys[SDLK_LCTRL] && Editor::keys[SDLK_s]) {
        ...
        content = Convert_SettingsToString(settings) + Convert_PixelsArraysToString(pixels);
        pixels = Convert_StringToPixelsArrays(content);
        Save();
        Render();
            ...
            if (!EqualColors(base,pen)) {
                ...
                do {
                    ...
                    if (posX == 0 && posY == 0) {
                        ...
                        if (EqualColors(pixels[posX+1][posY].color,base) && !VectorPixelHas(colored,&pixels[posX+1][posY])) colored.push_back(&pixels[posX+1][posY]);
                        if (EqualColors(pixels[posX][posY+1].color,base) && !VectorPixelHas(colored,&pixels[posX][posY+1])) colored.push_back(&pixels[posX][posY+1]);
