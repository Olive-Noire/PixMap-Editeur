...
std::vector<std::vector<Pixel>> Convert_StringToPixelsArrays(std::string pixels) {
    ...
    for (unsigned int i = 0;i < lines.size();i++) {
        ...
        pxs.push_back(Convert_StringToPixelsLine(AssemblyStringVector(linesRange,' ')));
...
Pixel Convert_StringToPixel(std::string px) {
    ...
    return Pixel(Convert_StringToNumber(s[0]),Convert_StringToNumber(s[1]),Convert_StringToNumber(s[2]));
